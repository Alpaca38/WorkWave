...
final class SLPResponseSerializer<T: Decodable, ErrorMapper: ResponseErrorMapper, S: Sequence>: ResponseSerializer where S.Iterator.Element == Int {
    ...
    func serialize(request: URLRequest?, response: HTTPURLResponse?, data: Data?, error: Error?) throws -> T {
        ...
        if !acceptableStatusCode.contains(response.statusCode) {
            let errorCode = try decoder.decode(ErrorResponse.self, from: data).errorCode
    ...
    private func mappingError(errorCode: String) throws {
        if let requestError = responseErrorMapper?.mappingError(errorCode) {
            ...
            throw SLPCommonError.init(rawValue: errorCode) ?? NetworkError.unknown
        }
    }
